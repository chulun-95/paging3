...
class CatRemoteMediator(
    ...
    override suspend fun load(
        ...
        Log.d("CatRemoteMediator", "load() called with: pageKeyData $pageKeyData")
        ...
        try {
            ...
            Log.d("CatRemoteMediator", "load() called with: page ${page}   response = ${response.size}")
            ...
            Log.d("CatRemoteMediator", "load() called with: result")
    ...
    private suspend fun getKeyPageData(loadType: LoadType, state: PagingState<Int, Cat>): Any {
        Log.d("CatRemoteMediator", "getKeyPageData() called with: loadType = $loadType, state = ${state.pages.size}")
        return when (loadType) {
            LoadType.REFRESH -> {
                val remoteKeys = getRemoteKeyClosestToCurrentPosition(state)
                Log.d("CatRemoteMediator", "REFRESH getKeyPageData() called with: key = $remoteKeys")
                remoteKeys?.nextKey?.minus(1) ?: STARTING_PAGE_INDEX
            }
            LoadType.APPEND -> {
                val remoteKeys = getLastRemoteKey(state)
                Log.d("CatRemoteMediator", "APPEND getKeyPageData() called with: key = $remoteKeys")
